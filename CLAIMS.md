...
| `subjects(key=…)` is declarable in a policy file, and the boundary removes a refused element from a document it still serves — with a projection that would hide an element's marks refused outright, because no query can remove an array element | `tests/test_a_subject_is_declarable_and_the_wire_redacts.py` |
| An embedded subject — a chapter in a book, a comment on a ticket — is refused without refusing its parent, on the read path the proxy uses, and the redaction is never silent | `tests/test_an_embedded_subject_is_refused_without_its_parent.py` |
