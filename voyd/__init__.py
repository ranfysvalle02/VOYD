...
TTL index, with the delete event itself reclaiming the bytes.

Three primitives, and everything else is mechanics:
...
**Guard** -- an access policy on the scope: a passcode, a read limit, enforced
on queries and on bytes alike.
